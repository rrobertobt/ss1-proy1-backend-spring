...
public class UserController {
    ...
    public ResponseEntity<ApiResponseDto<List<AddressResponseDto>>> getUserAddresses(Authentication authentication) {
        Integer userId = getCurrentUserId(authentication);
        List<AddressResponseDto> addresses = userService.getUserAddresses(userId);

        return ResponseEntity.ok(ApiResponseDto.success(new AddressListWrapper(addresses)));
    }
    ...
    public ResponseEntity<ApiResponseDto<CreditCardListWrapper>> getUserCreditCards(Authentication authentication) {
        Integer userId = getCurrentUserId(authentication);
        List<CreditCardResponseDto> cards = userService.getUserCreditCards(userId);

        return ResponseEntity.ok(ApiResponseDto.success(new CreditCardListWrapper(cards)));
    }
    ...
    public ResponseEntity<ApiResponseDto<Disable2FAResponseDto>> disable2FA(
        ...
        return ResponseEntity.ok(ApiResponseDto.success("Autenticación de dos factores deshabilitada", response));
    }
